...
For Linux environments, use the `prepare.sh` script to create the necessary database initialization file and self-signed certificates. 

For Windows environments, use the `prepare.ps1` script instead, which is adapted for compatibility with Windows systems.

`prepare.sh`/`prepare.ps1` is a small script that creates `./init/initdb.sql` by downloading the docker image `guacamole/guacamole` and start it like this:
...
## reset.sh / reset.ps1
To reset everything to the beginning, use `reset.sh` on Linux or `reset.ps1` on Windows.
